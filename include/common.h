...
struct LocalCfg1{
  ...
  #ifdef _DOORBELL_SUPPORT   //第8防区作为有线门铃使用  20130618
    uint8_t DoorBell_Support;
  #endif

  #ifdef _PON_SUPPORT   //PON 支持 20140314
    uint8_t Infrared_Support;  //红外检测启用
  #endif  
};
...
struct PonSearch1{
  ...
  uint16_t tx_pwr;
  uint16_t rx_pwr;
  uint8_t Pon_Status; // 0 - ONU未启动  1 - 未插光纤  2 - 注册过程  3 - 未注册  4 - 注册成功
  uint8_t Status0_Num;  //ONU未启动计数
};
#endif
...
#define CommonH
  ...
  char UdpPackageHead[15];
    ...
    char PonPackageHead[15];
    struct PonSearch1 PonSearch;
    uint8_t LoginMode;    //1 -- 超级密码
  #endif
...
#else
  #ifdef _PON_SUPPORT 	//添加PON支持 20190403 ，by zz
   extern char PonPackageHead[15];
   extern struct PonSearch1 PonSearch;
   extern uint8_t LoginMode;    //1 -- 超级密码
  #endif
  ...
  #ifdef _ADDTABLE_SUPPORT  //地址表支持
    extern struct AddrTable1 AddrTable;
  #endif
